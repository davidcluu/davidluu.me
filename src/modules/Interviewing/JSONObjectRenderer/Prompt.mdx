...
Given an input JavaScript object, implement a component that renders the object. *For simplicity, only values that are valid JSON data types (null, boolean, number, string, array, object) need to be considered.* Objects should be rendered as follows:
* null - `null`
...
In addition, non-flat elements (arrays and objects) should have a button that expands and collapses the element and all of its children. *Children arrays and objects can remain collapsed/uncollapsed when the parent array or object is expanded again.* When collapsed, the object should instead be rendered as follows:
...
The unicode characters ⮞ (expand; U+2B9E) and ⮟ (collapse; U+2B9F) can be used as the text for the expand/collapse buttons.
