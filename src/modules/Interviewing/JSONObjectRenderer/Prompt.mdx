...
export const EXPANDED_BUTTON_TEXT = '⬇️';
export const COLLAPSED_BUTTON_TEXT = '➡️';

# JSON Object Renderer

Given an input JavaScript object, implement a component that renders the object. <HideIf comparision={Comparision.GreaterThan} level={1}>For simplicity, only values that are valid JSON data types (null, boolean, number, string, array, object) need to be considered.</HideIf> Objects should be rendered as follows:
* null - `null`
...
***Note:*** <Var value={EXPANDED_BUTTON_TEXT} /> (expanded) and <Var value={COLLAPSED_BUTTON_TEXT} /> (collapsed) can be used as the text for the expand/collapse buttons. <HideIf comparision={Comparision.GreaterThan} level={1} />
